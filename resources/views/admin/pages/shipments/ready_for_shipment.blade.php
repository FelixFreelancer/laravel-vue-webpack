...
@section('contentFooter')
    <script>
        ...
        $(document).on('click', '.readyForShip', function (e) {
            e.preventDefault();
            $("#id").val($(this).data('id'));
        ...
        $(document).on('click', '#submit_shipment', function (e) {
            ...
            if($("#tracking_no").val() != ''){
              $.ajax({
                dataType: 'json',
                method: 'post',
                data: 'tracking_no='+$("#tracking_no").val()+'&id='+$("#id").val(),
                url: adminURL + "shipment/ready-for-shipment",
              ...
              $(".tracking_no_error").html("Tracking Number is required");
            }
        });
